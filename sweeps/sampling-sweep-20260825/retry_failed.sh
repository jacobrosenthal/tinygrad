#!/usr/bin/env bash
set -uo pipefail
# Retries the 3 legs that failed in the first run_sweep.sh pass (2026-08-25):
#   fork_qwen_recommended  -- hung past the 30min startup timeout; fixed by computing top_p over
#                              just the top_k candidates instead of a full ~150K-vocab sort (see
#                              model.py _apply_top_pk's updated docstring)
#   plain_temp10 / plain_qwen_recommended -- used the wrong model file (UD quant, GGML type 11 not
#                              supported by the plain tinygrad loader); now use the regular Q4_0 file
# Appends to the same results.csv from the first run (its header/5 successful legs stay intact).
...
run_leg() {
  ...
  local logfile="$LOGDIR/${leg}.retry.server.log"
  ...
  for task in prefill prose code; do
    ...
    echo "$out" | tee "$LOGDIR/${leg}.retry.${task}.json"
