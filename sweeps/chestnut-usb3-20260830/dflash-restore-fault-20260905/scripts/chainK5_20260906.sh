#!/bin/bash
# After chainK4: rerun the chunk-width numerics test (script fixed: block types from the block classes) on KFD.
#   WAIT_PID=<chainK4 pid> bash chainK5_20260906.sh
S=$(cd "$(dirname "$0")" && pwd); TOP=/home/jacob/z/tinygrad; B=$TOP/sweeps/chestnut-usb3-20260830; LOGD=$S/../logs; CL="$LOGD/$(date +%Y%m%d-%H%M%S)-chainK5.log"
...
log "chainK5 start: waiting for pid ${WAIT_PID:-none}"
