#!/bin/bash
# DROP DETECTOR on the CURRENT (unfixed-F0) firmware. Fault-prone config (serialize OFF, spin 0) + USB_VERIFY_WRITES=2:
...
LOG="$SD/dflash-dropdetect2.log"; RES="$SD/dflash-dropdetect2-result.txt"; : > "$LOG"; : > "$RES"
...
echo "DROP DETECTOR (unfixed firmware, serialize OFF, spin 0, USB_VERIFY_WRITES=2) $(date +%T)" | tee -a "$RES"
