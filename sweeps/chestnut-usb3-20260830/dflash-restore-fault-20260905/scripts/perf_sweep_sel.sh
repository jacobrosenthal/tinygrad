...
# Post-fix throughput sweep on RESTORED instances (the once-faulting config). For each config: one warmup (saves the LLM cache
...
SUM="$LOGD/$(date +%Y%m%d-%H%M%S)-perfsel-summary.txt"
...
# name | extra env
CFGS=("dflash-b8-sel|DFLASH=$DFLASH DFLASH_BLOCK=8 DFLASH_EAGER_SEL=1 DFLASH_XCTX=0"
      "dflash-b6-sel|DFLASH=$DFLASH DFLASH_BLOCK=6 DFLASH_EAGER_SEL=1 DFLASH_XCTX=0"
      "dflash-b8-nosel|DFLASH=$DFLASH DFLASH_BLOCK=8 DFLASH_EAGER_SEL=0 DFLASH_XCTX=0")
...
for cfg in "${CFGS[@]}"; do
  ...
  LOG="$LOGD/$(date +%Y%m%d-%H%M%S)-perfsel-$name.log"
