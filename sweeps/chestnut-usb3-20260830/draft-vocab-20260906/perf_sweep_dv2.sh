...
SUM="$LOGD/$(date +%Y%m%d-%H%M%S)-dv2-summary.txt"
...
# name | extra env
CFGS=("k3-tg16-old|MTP_K=3 GEMV_TG=16" "k3-fa|MTP_K=3 FAST_ARGMAX=1" "k3-dv64k-fa|MTP_K=3 MTP_DRAFT_VOCAB=65536 FAST_ARGMAX=1" "k4-dv64k-fa|MTP_K=4 MTP_DRAFT_VOCAB=65536 FAST_ARGMAX=1")
...
for cfg in "${CFGS[@]}"; do
  ...
  LOG="$LOGD/$(date +%Y%m%d-%H%M%S)-dv2-$name.log"
