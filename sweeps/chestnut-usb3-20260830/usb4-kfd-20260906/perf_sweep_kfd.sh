...
# KFD/USB4 variant of dflash-restore-fault-20260905/scripts/perf_sweep.sh: same protocol, DEV from $DEVSTR (default KFD+AMD:LLVM).
...
LOGD=$TOP/sweeps/chestnut-usb3-20260830/usb4-kfd-20260906/logs; mkdir -p "$LOGD"
MODEL=/home/jacob/models/Qwen3.8-27B-UD-Q4_K_XL.gguf; DFLASH=/home/jacob/models/Qwen3.8-27B-DFlash2-Q4_K_M.gguf
SUM="$LOGD/$(date +%Y%m%d-%H%M%S)-perf-summary.txt"
...
# name | extra env
CFGS=("mtp-k3|MTP_K=3" "mtp-k4|MTP_K=4" "mtp-k5|MTP_K=5"
      "dflash-x0|DFLASH=$DFLASH DFLASH_BLOCK=6 DFLASH_NOSEL=1 DFLASH_XCTX=0"
      "dflash-x16|DFLASH=$DFLASH DFLASH_BLOCK=6 DFLASH_NOSEL=1 DFLASH_XCTX=16")
...
for cfg in "${CFGS[@]}"; do
  ...
  LOG="$LOGD/$(date +%Y%m%d-%H%M%S)-perf-$name.log"
  echo "=== $name: $extra ===" | tee -a "$SUM"
  for RUN in $(seq 0 $R); do   # run 0 = warmup (saves cache), 1..R = restores
    killsrv; sleep 3; cd "$TOP"; before=$(grep -ac "llm cache: loaded" "$LOG" 2>/dev/null)
    env $extra WAVEDUMP=1 LLM_CACHE=1 DEV=${DEVSTR:-KFD+AMD:LLVM} MAX_T=12 ATTN_QT=8 NGRAM_DRAFT=0 PYTHONUNBUFFERED=1 \
